...
### 内容参数讲解
```
IN前的@符号：代表zone的意思，就是定义zone "***"这个里面的区域名（通常直接写域名），定义这条数据是给谁的
IN后的@符号：这条数据里写的服务器是谁。比如有主备dns服务器，那主dns服务器是ns1.aaa.rl，从服务器是ns2.aaa.rl，
TTL：记录缓存时间长度，不写单位默认为秒，这里默认是1天
IN：代表internet记录的意思，基本照抄为IN就行
SOA：起始授权记录。一个区域解析库有且只能有一条SOA记录，必须位于解析库的第一条记录
A：即A记录，将域名映射到ipv4地址
AAAA：将域名映射到ipv6地址
